...
import { ArrowLeft, User, Edit, Save, X, Mail, Phone, Calendar, DollarSign, AlertTriangle, TrendingUp, Clock, FileText, Award } from 'lucide-react'
...
export default function EmployeeDetailsPage() {
  ...
  // GraphQL hooks
  const { data: employeeData, refetch } = useQuery(GET_EMPLOYEE, { variables: { id: employeeId } })
  const { data: schedulesData } = useQuery(GET_WORK_SCHEDULES, { variables: { employee_id: employeeId } })
  const { data: contractsData } = useQuery(GET_CONTRACTS, { variables: { employee_id: employeeId } })
  ...
  // Render: keep a stable return (no early return before hooks)
        ...
        <div className="glass-card backdrop-blur-futuristic p-4 sm:p-6 lg:p-8 text-white shadow-2xl animate-fade-in relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 relative z-10">
            <div className="flex items-center space-x-3 sm:space-x-4 min-w-0 flex-1">
              <Button variant="ghost" size="sm" onClick={() => router.back()} className="text-white hover:bg-white/20 p-1 sm:p-2 flex-shrink-0">
              ...
              <div className="min-w-0 flex-1">
                <h1 className="text-lg sm:text-2xl lg:text-3xl font-bold mb-1 truncate bg-gradient-to-r from-white via-blue-100 to-purple-100 bg-clip-text text-transparent" dir="auto">
            ...
            <div className="flex space-x-2 w-full sm:w-auto">
              {!isEditing ? (
                <Button onClick={handleEdit} className="bg-white/20 hover:bg-white/30 text-white border-white/30 text-xs sm:text-sm flex-1 sm:flex-none">
                  <Edit className="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2" /> {t('edit')}
                </Button>
              ) : (
                <>
                  <Button onClick={handleSave} className="bg-green-600 hover:bg-green-700 text-white text-xs sm:text-sm flex-1 sm:flex-none">
                    <Save className="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2" /> {t('save')}
                  </Button>
                  <Button onClick={handleCancel} className="bg-red-600 hover:bg-red-700 text-white text-xs sm:text-sm flex-1 sm:flex-none">
          ...
          <>
              ...
              <CardContent className="p-4 sm:p-6 pt-0">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Badge variant={employee.status === 'active' ? 'default' : 'secondary'} className="text-xs sm:text-sm" dir="auto">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm sm:text-base font-semibold text-foreground">{formatDate(employee.created_at)}</span>
                    </div>
                  </div>
                </div>
              </CardContent>
                  ...
                  {[
                    ...
                    <div key={item.key} className="space-y-2">
                      ...
                      {isEditing ? (
                        ...
                        <div className="text-lg sm:text-xl font-bold text-foreground">{formatCurrency(employee[item.key] || 0)}</div>
                      )}
                    </div>
                  ))}
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
              <Card className="glass-card backdrop-blur-futuristic border-0 shadow-2xl bg-gradient-to-br from-card to-card/80">
                ...
                <CardContent className="p-4 sm:p-6 pt-0 space-y-4 sm:space-y-6">
                  <div className="text-center">
                    <div className="text-2xl sm:text-4xl font-bold text-foreground mb-2">{formatNumber(performanceScore)}%</div>
                    <Badge
                      variant={performanceScore >= 80 ? 'default' : performanceScore >= 60 ? 'secondary' : 'destructive'}
                      className="text-xs sm:text-sm"
                      dir="auto"
                    >
                      {performanceScore >= 80 ? t('excellent') : performanceScore >= 60 ? t('good') : t('needs_improvement')}
                    </Badge>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    <div className="text-center">
                      <div className="text-lg sm:text-xl font-bold text-foreground">{formatNumber(Number(totalHours.toFixed(0)))}h</div>
              ...
              <Card className="glass-card backdrop-blur-futuristic border-0 shadow-2xl bg-gradient-to-br from-card to-card/80">
                <CardHeader className="p-4 sm:p-6">
                  <CardTitle className="flex items-center text-base sm:text-lg">
                    <AlertTriangle className="w-4 h-4 sm:w-5 sm:h-5 mr-2" /> <span dir="auto">{t('disciplinary_data')}</span>
                  </CardTitle>
                </CardHeader>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          {isEditing ? (
                            ...
                            <>
                              <span className="text-sm sm:text-lg font-bold text-foreground">{formatNumber(employee[item.key] || 0)}</span>
                      ...
                      <div className="text-right">
                        {isEditing ? (
                          <Input
                            ...
                            onChange={(e) => setEditData({ ...editData, tenu_de_travail: Number.parseInt(e.target.value) || 0 })}
                            className="w-16 sm:w-20 h-6 sm:h-8 text-xs sm:text-sm"
                          />
                        ) : (
                          <span className="text-sm sm:text-lg font-bold text-foreground">{formatNumber(employee.tenu_de_travail || 0)}</span>
                        )}
                      </div>
                    </div>
                  </div>
